...
public class UIScrollList : MonoBehaviour, IUIObject
{
	...
	public void InsertItem(IUIListObject item, int position, string text, bool doEasing)
	{
		...
		{
			SetVisibleRecursive(((Component)item).transform, true);
		...
		if(!gameObject.active)
			SetVisibleRecursive(((Component)item).transform, false);
		...
		if (clipContents)
		{
			SetVisibleRecursive(item.transform, false);
			/*item.Hide(true);
			if (!item.Managed)
...
// 						}
				...
				if (firstItem != null)
				{
					...
					SetVisibleRecursive(firstItem.transform, true);
					...
					if (firstItem.BottomRightEdge.x + itemOffset < rightVisibleEdge)
					{
						...
						for (index = firstItem.Index + 1; index < items.Count; ++index)
						{
							...
							if (items[index].BottomRightEdge.x + itemOffset >= rightVisibleEdge)
							{
								...
								SetVisibleRecursive(items[index].transform, true);
								/*if (!items[index].gameObject.active)
...
// 						}
		...
		if (visibleItems.Count > 0)
		{
			   ...
			   visibleItems[visibleItems.Count - 1].Index < firstItem.Index)
			{
				...
				{
					SetVisibleRecursive(visibleItems[i].transform, false);
	...
	private void SetVisibleRecursive(Transform tr, bool isVisible)
	{
		if (tr.renderer != null)
			tr.renderer.enabled = isVisible;
		...
		{
			SetVisibleRecursive(child, isVisible);
		}
	}
...
*/
	...
	public void RemoveItem(int index, bool destroy, bool doEasing)
	{
		...
		else
		{
			...
			// Deactivate:
			SetVisibleRecursive(items[index].transform, false);
			//items[index].gameObject.SetActiveRecursively(false);
		}
	...
	public void ClearList(bool destroy)
	{
		...
		for(int i=0; i<items.Count; ++i)
		{
			...
			if (destroy)
				...
				SetVisibleRecursive(items[i].transform, false);
